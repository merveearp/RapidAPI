...
namespace Hotel.WEBUI.Areas.Admin.Controllers
{
    ...
    public class ServiceController : Controller
    {
        ...
        public async Task<IActionResult> AddService(CreateServiceDto serviceDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5178/api/Service/", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5178/api/Service/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5178/api/Service/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDto serviceDto)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(serviceDto);
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:5178/api/Service/", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
